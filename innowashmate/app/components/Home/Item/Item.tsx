...
const Item: React.FC = () => {
    ...
    useEffect(() => {
        const fetchMachineDetails = async () => {
            try {
                const schedulesCollection = doc(FIRESTORE_DB, 'schedules', id.split('-').slice(0, 2).join('-')); // Получаем ссылку на документ
    ...
    const handleCancel = async () => {
        if (machineDetails) {
            ...
            try {
                const docRef = doc(FIRESTORE_DB, 'schedules', id.split('-').slice(0, 2).join('-'));
